...
plot(diamonds$carat, diamonds$price) # alternative way of plotting scatterplots
# using the variables directly

# 7.1.3 Boxplots - Greatly debated among statistics community.
# The thick middle line represents the median and the box is bounded by the 1st
# and 3rd quartiles
...
# NOTE that while 50% of the data are very visible in the box, the remaining
# 50% of the data are not really displayed
boxplot(diamonds$carat)

##############################################################################
# 7.2 ggplot2
# ggplot2 and lattice were built to make graphing easier, where ggplot2 had
# overexceeded lattice
# syntax hard to grasp, but its easier to delineate data by color, shape or
...
# density plots show the probability of observations falling within a sliding
# window
...
# changing to violin plots - this plot is similar to boxplots except the
# curviness gives a
# sense of density of the data, thus providing more information than the
# straight sides of ordinary boxplots
ggplot(diamonds, aes(y = carat, x = cut)) + geom_violin()
ggplot(diamonds, aes(y = carat, x = cut)) + geom_point() + geom_violin()
ggplot(diamonds, aes(y = carat, x = cut)) + geom_violin() + geom_point()
# different order of positioning

# 7.2.4 ggplot2 Line Graphs
# often used when one variable has a certain continuity, but not always
# necessary because there is often
# a good reason to use a line with a categorical data
ggplot(economics, aes(x = date, y = pop)) + geom_line()

# A common task for line plots is displaying a metric over the course of a
# year for many years
# e.g. prepare the economics data by using Wickham's lubridate package which
# has functions to manipulate dates
# need to create 2 new variables, year and month, but to simplify it we'll
# subset to only start from 2000
require(lubridate)

# create year and month variables
economics$year <- year(economics$date)

# the label argument to month means that the results should be the names of
# the month instead of the number
economics$month <- month(economics$date, label = TRUE)

# subset the data
# the which function returns the indices of observations where the tested
# condition was TRUE
...
# 7.2.5 Themes
# Great part of ggplots is the ability to use themes to change easily the way
# plots look
require(ggthemes)
